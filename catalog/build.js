...
for (const skillPath of skillFiles) {
  ...
  let outPath;
  if (depth === 1) {
    // domain manifest (e.g. tech/SKILL.md) — skip, hand-written index exists
    continue;
  } else {
    // leaf skill or subdomain manifest → flatten to parent dir + leaf name
    // tech/cloudflare/workers → catalog/tech/cloudflare/workers.html
    // tech/cloudflare/ai/workers-ai → catalog/tech/cloudflare/workers-ai.html (flatten ai/ level)
    ...
    if (parentDir.includes('ai') && parentDir.length > 2) {
      // e.g. tech/cloudflare/ai/workers-ai → catalog/tech/cloudflare/workers-ai.html
  ...
  const outDepth = outPath.replace(CATALOG + '/', '').split('/').length; // depth from catalog root
